...
@Component({
  selector: 'app-contactusqueries',
  templateUrl: './contactusqueries.component.html',
  styleUrls: ['./contactusqueries.component.css']
})
export class ContactusqueriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    firebase.firestore().collection("ContactUs").get().then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
          ...
          arr.push({"name":doc.data().name,"email":doc.data().email,"messgae":doc.data().messgae,"id":doc.id})
...
update(name,url,description,id)

{ 
console.log(name,url,description)

firebase.firestore().collection("ContactUs").doc(id).update({ 

  answer:description
  
}).then(() =>{
  this._snackBar.open("Published Answered", "", {
    duration: 2000,
...
delete(name,url,description,id)

{ 
console.log(name,url,description,id)

firebase.firestore().collection("ContactUs").doc(id).delete().then(() => {
  this.ngOnInit()
  this._snackBar.open("Deleted Question", "", {
    duration: 2000,
